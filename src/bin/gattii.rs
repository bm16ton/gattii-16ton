...
//    line_ending_map: HashMap<String, u32>,
...
static BAUD_RATES: [&'static str; 16] = [
	"2000000",
	"1000000",
    "921600",
	"576000",
...
static NEWLINE: [&'static str; 3] = [
    "\n",
    "\r",
    "\r\n",
];
static DEFAULT_NEWLINE: &'static str = "\n";
...
fn main() {
    ...
    let matches = App::new("Gattii")
                      ...
                      .arg(Arg::with_name("newline")
                          .long("newline")
                          .help("line return options")
                          .takes_value(true)
                          .requires("port")
                          .possible_values(&NEWLINE)
                          .default_value_if("newline", None, DEFAULT_NEWLINE))
                      .get_matches();
...
fn ui_init() {
    ...
    if let Ok(mut ports) = list_ports() {
        ...
        if !ports.is_empty() {
            ...
            ports_dropdown.append(None, "/dev/tty63");
			ports_dropdown.append(None, "/dev/tty62");
			ports_dropdown.append(None, "/dev/ttyESP");
			ports_dropdown.append(None, "/dev/ttyBLE");
			ports_dropdown.append(None, "/dev/ttyCUST");
			ports_dropdown.set_sensitive(true);
			ports_dropdown.set_active(0);
        } else {
            ports_dropdown.append(None, "No USB ports found");
            ports_dropdown.append(None, "/dev/tty63");
			ports_dropdown.append(None, "/dev/tty62");
			ports_dropdown.append(None, "/dev/ttyESP");
			ports_dropdown.append(None, "/dev/ttyBLE");
			ports_dropdown.append(None, "/dev/ttyCUST");
            ports_dropdown.set_active(1);
            ports_dropdown.set_sensitive(true);
        }
    } else {
...
/*    let newline_label = gtk::Label::new("Newline:");
    newline_label.set_halign(gtk::Align::End);
    popover_container.attach(&newline_label, 0, 4, 1, 1);
    let newline_dropdown = gtk::ComboBoxText::new();
    let mut newline_dropdown_map = HashMap::new();
    for (i, b) in NEWLINE.iter().enumerate() {
        newline_dropdown_map.insert(b.to_string(), i as u32);
        newline_dropdown.append(None, b);
    }
    newline_dropdown.set_active(newline_dropdown_map[DEFAULT_NEWLINE]);
    popover_container.attach(&newline_dropdown, 1, 4, 1, 1);
*/
    let separator = gtk::SeparatorMenuItem::new();
    popover_container.attach(&separator, 0, 4, 2, 1);
    let line_ending_label = gtk::Label::new("Newline:");
    ...
    let send_image = gtk::Image::new_from_file("/etc/gattii/upload.svg");
    ...
    let save_image = gtk::Image::new_from_file("/etc/gattii/download.svg");
    ...
    css_provider.load_from_path("/etc/gattii/style.css").expect("Failed to load CSS stylesheet");
...
/*
            let line_ending_dropdown_changed_signal = line_ending_dropdown.connect_changed(move |s| {
...
*/
    ...
    // Configure the newline dropdown callback
/*
    newline_dropdown.connect_changed(|s| {
        let newline = match s.get_active_text() {
            Some(ref x) if x == "\n" => Some(newline_n::N),
            Some(ref x) if x == "\r" => Some(Newline::R),
            Some(ref x) if x == "\r\n" => Some(Newline::RN),
            Some(_) | None => unreachable!(),
        };
        if let Some(newline) = newline {
            GLOBAL.with(|global| if let Some((_, ref serial_thread, _)) = *global.borrow() {
                    match serial_thread.send_port_change_newline_cmd(newline) {
                        Err(GeneralError::Parse(_)) => {
                            unreachable!();
                        }
                        Err(GeneralError::Send(_)) => {
                            error!("Error sending newline change \
                                        command to child thread. Aborting.")
                        }
                        Ok(_) => (),
                    }
...
//        newline_dropdown: newline_dropdown.clone(),
//        newline_map: newline_dropdown_map,
...
//	line_ending_map: line_ending_dropdown_map,
...
fn view_populate_popup(text_view: &gtk::TextView, popup: &gtk::Widget) {
	...
	let newline_none = gtk::RadioMenuItem::new_with_label_from_widget(&newline_n, "\\0");
        ...
        newline_none.connect_toggled(|w| {
            GLOBAL.with(|global| {
                if let Some((.., ref mut state)) = *global.borrow_mut() {
                    // The toggle signal triggers on activation and deactivation, so only respond
                    // to activations here.
            ...
            match serial_thread.from_port_chan_rx.try_recv() {
                ...
                Ok(SerialResponse::OpenPortError(s)) => {
                    ...
                    if ports.is_empty() {
                        ui.ports_dropdown.append(None, "No ports found");
			            		ui.ports_dropdown.append(None, "/dev/tty63");
						ui.ports_dropdown.append(None, "/dev/tty62");
						ui.ports_dropdown.append(None, "/dev/ttyESP");
						ui.ports_dropdown.append(None, "/dev/ttyBLE");
						ui.ports_dropdown.append(None, "/dev/ttyCUST");
                ...
                Ok(SerialResponse::PortsFound(ports)) => {
                    ...
                    if replace {
                        ...
                        ui.ports_map.clear();
                        if ports.is_empty() {
                            ui.ports_dropdown.append(None, "No USB ports found");
                            ui.ports_dropdown.append(None, "/dev/tty63");
			                ui.ports_dropdown.append(None, "/dev/tty62");
			                ui.ports_dropdown.append(None, "/dev/ttyESP");
			                ui.ports_dropdown.append(None, "/dev/ttyBLE");
